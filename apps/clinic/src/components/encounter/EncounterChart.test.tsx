...
describe('EncounterChart', () => {
  ...
  test('renders structured clinical note fields', async () => {
    setup();
    ...
    const textarea = screen.getByRole('textbox', { name: 'Additional clinical notes' });
  ...
  describe('signing functionality', () => {
    ...
    test('signs without locking - note becomes immutable', async () => {
      const user = userEvent.setup();
      ...
      // Any signature creates an immutable note boundary; later changes use addenda.
      await waitFor(
        () => {
    ...
    test('chart note is read-only when signed but not operationally locked', async () => {
      const mockProvenance: Provenance = {
        resourceType: 'Provenance',
        id: 'provenance-1',
        target: [createReference(finishedEncounter)],
      ...
      // Operational lock controls encounter completion; the signature itself controls note immutability.
      await waitFor(() => {
        const chartNoteCard = screen.getByText('Fill chart note').closest('.mantine-Card-root');
        const textarea = chartNoteCard?.querySelector('textarea');
        expect(textarea).toBeDisabled();
