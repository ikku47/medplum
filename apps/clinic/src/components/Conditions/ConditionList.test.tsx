...
describe('ConditionList', () => {
  ...
  test('displays modal form fields', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Diagnosis (SNOMED CT)')).toBeInTheDocument();
      expect(screen.getByText('ICD-10 code (optional)')).toBeInTheDocument();
  ...
  test('removes condition', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Remove diagnosis from encounter' }));
  ...
  test('creates a new condition with cholera ICD-10 code and active status', async () => {
    ...
    medplum.valueSetExpand = vi
      ...
      .mockImplementation(async (params: { url: string; filter?: string; count?: number }) => {
        if (params.url === 'http://snomed.info/sct?fhir_vs') {
          const choleraCodes = [
            {
              system: 'http://hl7.org/fhir/sid/icd-10-cm',
              code: 'A00.0',
    ...
    let icd10Input = icd10Inputs.find((input) => {
      ...
      return label?.textContent?.includes('Diagnosis (SNOMED CT)');
    });
      ...
      await waitFor(
        () => {
          ...
          const choleraCall = calls.find(
            (call) =>
              call[0]?.url === 'http://snomed.info/sct?fhir_vs' && call[0]?.filter?.toLowerCase().includes('cholera')
          );
