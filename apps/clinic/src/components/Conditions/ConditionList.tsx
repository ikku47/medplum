...
export const ConditionList = (props: ConditionListProps): JSX.Element => {
  const { patient, encounter, conditions, setConditions, onDiagnosisChange, disabled = false } = props;
  ...
  const handleRemoveDiagnosis = (condition: Condition): void => {
    if (!conditions) {
      return;
    }

    const remainingConditions = conditions.filter((item) => item.id !== condition.id);
    const updatedDiagnosis = encounter.diagnosis?.filter(
      (diagnosis) => diagnosis.condition?.reference !== getReferenceString(condition)
    );
    const reindexedDiagnosis = updatedDiagnosis?.map((diagnosis, index) => ({
      ...diagnosis,
      rank: index + 1,
    }));
