...
export const consentPolicyRuleMapping: Record<string, CodeableConcept> = {
  clinicRegistration: {
    coding: [
      {
        system: 'https://clinicbuddy.health/fhir/CodeSystem/consent-policy',
        code: 'clinic-registration-and-treatment',
        display: 'Clinic registration and treatment consent',
      },
    ],
  },
  hipaaNpp: {
...
export function getPatientAddress(answers: Record<string, QuestionnaireResponseItemAnswer>): Address | undefined {
  ...
  } else if (answers['state']?.valueString) {
  ...
  return Object.keys(patientAddress).length > 0
    ? { use: 'home', type: 'physical', country: 'IN', ...patientAddress }
    : undefined;
}
