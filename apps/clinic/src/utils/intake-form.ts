...
export async function onboardPatient(
  ...
  if (!project?.id) {
    throw new Error('An active ClinicBuddy tenant is required to register a patient.');
  }
  ...
  const gender = answers['gender']?.valueCoding?.code ?? answers['gender-identity']?.valueCoding?.code;
  if (gender) {
    patient.gender = gender as Patient['gender'];
  }

  if (answers['phone']?.valueString) {
    patient.telecom = [{ system: 'phone', use: 'mobile', value: normalizeIndiaMobile(answers['phone'].valueString) }];
  }

  if (answers['email']?.valueString) {
    patient.telecom = append(patient.telecom, {
      system: 'email',
      use: 'home',
      value: answers['email'].valueString.trim().toLocaleLowerCase('en-IN'),
    });
  }

  const abhaNumber = answers['abha-number']?.valueString;
  if (abhaNumber && answers['abha-consent']?.valueBoolean !== true) {
    throw new Error('Patient consent is required before storing an ABHA number.');
  }
  if (answers['consent-for-treatment-signature']?.valueBoolean !== true) {
    throw new Error('Registration and treatment consent is required to create a patient record.');
  }
  patient.identifier = createIndiaPatientIdentifiers(project.id, abhaNumber);
  ...
  const duplicatePatients = await findDuplicatePatients(medplum, patient);
  if (duplicatePatients.length > 0) {
    throw new DuplicatePatientError(duplicatePatients);
  }
  ...
  await addConsent(
    ...
    consentPolicyRuleMapping.clinicRegistration,
